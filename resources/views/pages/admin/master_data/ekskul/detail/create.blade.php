    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="addDetailEkskulModalLabel">Tambah Detail Ekskul</h5>
        ...
        <form action="{{ route('detailEkskul.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <label class="form-label">Ekstrakulikuler <span style="color:red">*</span></label>
                    <select class="form-control @error('id_ekskul') is-invalid @enderror" name="id_ekskul" id="id_ekskul" required>
                        <option selected disabled>Pilih ekskul</option>
                        @foreach ($dataEkskul as $dataEkskul)
                            <option value="{{ $dataEkskul->id }}">{{ $dataEkskul->judul }}</option>
                        @endforeach
                    </select>
                    @error('id_ekskul')
