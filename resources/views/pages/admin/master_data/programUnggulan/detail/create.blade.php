    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="addDetailProgramUnggulanModalLabel">Tambah Detail</h5>
        ...
        <form action="{{ route('detailProgramUnggulan.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <label class="form-label">Program Unggulan <span style="color:red">*</span></label>
                    <select class="form-control @error('id_program_unggulan') is-invalid @enderror" name="id_program_unggulan" id="id_program_unggulan" required>
                        <option selected disabled>Pilih program unggulan</option>
                        @foreach ($dataProgramUnggulan as $dataProgramUnggulan)
                            <option value="{{ $dataProgramUnggulan->id }}">{{ $dataProgramUnggulan->judul }}</option>
                        @endforeach
                    </select>
                    @error('id_program_unggulan')
