...
<div class="card">
    <div class="card-header">
        <h5>Data Modul (Kelas - {{ $kelasModul }})</h5>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($modul->file != null)
                                        @if(file_exists('repo/modul/'.$modul->id_kelas_master.'/'.$modul->id.'/'.$modul->file))
                                            <a href="{{ asset('repo/modul/'.$modul->id_kelas_master.'/'.$modul->id.'/'.$modul->file) }}" class="btn btn-info btn-sm" target="_blank">
                                                Preview
                                            </a>
                                        @endif
                                    @endif
                                ...
                                @if(auth()->user()->jenis != 'siswa')
                                    <td class="table-action" width="10%">
                                        <a href="{{ route('dataModul').'/'.$modul->id }}" class="btn btn-primary btn-sm">
...
@include('pages.admin.bank_data.modul.summaryCreate')
